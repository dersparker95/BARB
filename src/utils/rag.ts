...
// 🔥 BLINDAJE: Interfaces estrictas para reutilización y seguridad de tipado
...
// Base de datos de emergencia offline intacta
...
export function tokenize(t: string): string[] {
  // Lógica intacta: Normaliza, quita tildes, pasa a minúsculas y filtra palabras clave
...
export function sourcesFromChunks(chunks: LocalChunk[], docName: string = 'Offline Database'): SourceHit[] {
  // 🔥 MEJORA: Validamos si realmente necesita los puntos suspensivos y aceptamos nombres dinámicos
  return chunks.map(c => ({ 
